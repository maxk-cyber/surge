...
import { PrismWall } from "@/components/ui/PrismWall";
...
function ShowroomDock() {
  return (
    <nav
      ...
      className="fixed inset-x-0 bottom-4 z-50 mx-auto flex w-[min(94vw,680px)] items-center justify-center rounded-full border border-white/10 bg-black/55 p-1.5 shadow-[0_18px_80px_rgba(0,0,0,0.55)] backdrop-blur-2xl"
    >
      {[
        ["Brief", "#brief"],
        ["Pack", "#pack"],
    ...
    <ClickSpark motion={motionLevel}>
      <main className={cn("relative min-h-screen overflow-hidden bg-gradient-to-b pb-28", vibeMode.wash)}>
          ...
          <AnimatedReveal motion={motionLevel}>
            <div>
              ...
              <p className="mt-6 max-w-2xl font-body text-sm leading-7 text-secondary/90 md:text-base">
                Browse the roster as a product-grade collectible interface: open a contraband pack,
                spin the generated portrait dome, inspect the prism wall, favorite your fighters, and
                copy shareable card summaries from the deck.
              </p>
        ...
        <section className="relative z-10 mx-auto grid w-full max-w-7xl gap-4 px-5 md:grid-cols-4 md:px-8">
          {[
            ["Roster", rosterStats.total, "Generated fighters with static-export safe art"],
        ...
        <PrismWall vibe={vibe} motionLevel={motionLevel} />
